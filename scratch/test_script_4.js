...
function selectGrade(grade) {
  ...
  if (grade === 6) {
    ...
    if (title) title.textContent = 'डिजिटल गुरु';
    if (subtitle) subtitle.textContent = 'कक्षा ६ : गणित — नेपाल सरकार पाठ्यक्रम विकास केन्द्र (CDC) नयाँ पाठ्यक्रम';
    ...
    if (title) title.textContent = 'डिजिटल गुरु';
    if (subtitle) subtitle.textContent = 'कक्षा ९ : विज्ञान तथा प्रविधि — नेपाल सरकार पाठ्यक्रम विकास केन्द्र (CDC) नयाँ पाठ्यक्रम';
...
// ================= CLASS 9 UNIT 2: CLASSIFICATION OF LIVING BEINGS SCRIPT =================

// State variables
let activeC9U2Tab = 'concepts';
let activeC9U2LabMode = 'kingdom';
let activeC9U2Kingdom = 'monera';
let activeC9U2Specimen = 'bacteria';
let activeC9U2DichotomousSpecimen = 'bacteria';
let activeC9U2DichotomousStep = 1;
let c9u2QuizAnswers = {};

// Tab Switching
function setTabC9U2(tab) {
  activeC9U2Tab = tab;
  const tabs = ['concepts', 'exercises', 'tiers', 'quiz'];
  tabs.forEach(t => {
    const view = document.getElementById(`c9u2-view-${t}`);
    const btn = document.getElementById(`c9u2-tab-${t}`);
    if (view) {
      if (t === tab) {
        view.classList.remove('hidden');
      } else {
...
// Virtual Science Lab Mode Switching
function setLabModeC9U2(mode) {
  activeC9U2LabMode = mode;
  const modes = ['kingdom', 'binomial', 'key'];
  modes.forEach(m => {
    const container = document.getElementById(`c9u2-lab-mode-${m}`);
    const btn = document.getElementById(`c9u2-btn-mode-${m}`);
    if (container) {
      if (m === mode) {
        container.classList.remove('hidden');
      } else {
    ...
    if (btn) {
      if (m === mode) {
        btn.className = 'px-3.5 py-2 rounded-xl text-xs font-bold transition bg-cyan-600 text-white shadow-sm cursor-pointer';
      } else {
  ...
  if (mode === 'kingdom') {
    selectKingdomC9U2(activeC9U2Kingdom);
  } else if (mode === 'binomial') {
    selectBinomialOrganism(document.getElementById('c9u2-binomial-select')?.value || 'human');
    buildCustomScientificName();
  } else if (mode === 'key') {
    renderDichotomousStep();
  }
}

// ================= 1. 5-KINGDOM TAXONOMIC EXPLORER DATA & ENGINE =================
const C9U2_KINGDOM_DATA = {
  monera: {
    id: 'monera',
    name: '१. जगत्: मोनेरा (Kingdom: Monera)',
    sub: 'Prokaryotic Microorganisms',
    badge: 'प्रोकारियोटिक सूक्ष्मजीव',
    badgeColor: 'bg-cyan-500/20 text-cyan-300 border-cyan-500/30',
    type: 'प्रोकारियोटिक (Prokaryotic) - न्युक्लियर झिल्ली नभएको अविकसित न्युक्लियोइड',
    organization: 'एककोषीय (Unicellular)',
    wall: 'पेप्टिडोग्लाइकन (Peptidoglycan) ले बनेको',
    nutrition: 'स्वपोषी (सायनोब्याक्टेरिया) वा परपोषी (मृतोपजीवी / परजीवी / सहजीवी)',
    motility: 'केहीमा फ्लाजेला (Flagella) द्वारा चाल',
    features: [
      'कोषमा माइटोकोन्ड्रिया, गल्गी बडी र इन्डोप्लाज्मिक रेटिकुलम जस्ता झिल्लीयुक्त अङ्गहरू हुँदैनन्।',
      '७०S प्रकृतिका साना राइबोजोम पाइन्छन्।',
      'प्रतिकूल वातावरण (धेरै तातो, नुनिलो, दलदल) मा पनि बाँच्न सक्छन्।',
      'अलैङ्गिक प्रजनन मुख्यतया द्विखण्डीकरण (Binary Fission) बाट हुन्छ।'
    ],
    examples: ['ब्याक्टेरिया (Bacteria)', 'सायनोब्याक्टेरिया / नोस्टक (Nostoc)', 'अजोटोब्याक्टर (Azotobacter)', 'एनाबेना (Anabaena)'],
    specimens: ['bacteria', 'nostoc']
  },
  protista: {
    id: 'protista',
    name: '२. जगत्: प्रोटिस्टा (Kingdom: Protista)',
    sub: 'Unicellular Eukaryotes',
    badge: 'एककोषीय युकारियोट्स',
    badgeColor: 'bg-teal-500/20 text-teal-300 border-teal-500/30',
    type: 'युकारियोटिक (Eukaryotic) - न्युक्लियर झिल्लीले घेरिएको स्पष्ट विकसित न्युक्लियस',
    organization: 'एककोषीय (Unicellular)',
    wall: 'प्रायः हुँदैन (डायटम्समा सिलिका भित्ता हुन्छ)',
    nutrition: 'स्वपोषी (युग्लिना), परपोषी होलोजोइक (अमिबा), परजीवी (प्लाज्मोडियम)',
    motility: 'नक्कली खुट्टा (Pseudopodia), सिलिया (Cilia), वा फ्लाजेला (Flagella)',
    features: [
      'कोषमा स्पष्ट न्युक्लियसका साथै माइटोकोन्ड्रिया, भ्याक्युल जस्ता सबै झिल्लीयुक्त अङ्गहरू हुन्छन्।',
      '८०S प्रकृतिका विकसित राइबोजोम हुन्छन्।',
      'प्रायः पानी (जलचर) वा ओसिलो ठाउँमा पाइन्छन्।',
      'प्रजनन अलैङ्गिक (द्विखण्डीकरण) र लैङ्गिक (संयोजन/कन्जुगेसन) दुवै विधिबाट हुन्छ।'
    ],
    examples: ['अमिबा (Amoeba)', 'युग्लिना (Euglena)', 'पारामेसियम (Paramecium)', 'प्लाज्मोडियम (Plasmodium)'],
    specimens: ['amoeba', 'euglena', 'paramecium']
  },
  fungi: {
    id: 'fungi',
    name: '३. जगत्: फन्जाई (Kingdom: Fungi)',
    sub: 'Multicellular Decomposers',
    badge: 'मृतोपजीवी विच्छेदक',
    badgeColor: 'bg-amber-500/20 text-amber-300 border-amber-500/30',
    type: 'युकारियोटिक (Eukaryotic) - स्पष्ट न्युक्लियस र पूर्ण कोषिकायुक्त',
    organization: 'बहुकोषीय (यिस्ट एककोषीय अपवाद हो)',
    wall: 'काइटिन (Chitin) ले बनेको कडा कोष भित्ता',
    nutrition: 'मृतोपजीवी (Saprotrophic), परजीवी वा सहजीवी (Lichens) - सोसेर लिने',
    motility: 'अचल (Non-motile)',
    features: [
      'हरितकण (Chlorophyll) पूर्ण रूपमा नहुने भएकाले प्रकाश-संश्लेषण गर्न सक्दैनन्।',
      'शरीर थाल्लोइड हुन्छ र मसिना धागो जस्ता हाइफी (Hyphae) को सञ्जाल माइसेलियमले बनेको हुन्छ।',
      'अतिरिक्त खाना ग्लाइकोजेन (Glycogen) को रूपमा सञ्चित गर्छन्।',
      'बीजाणु (Spores) वा बडिङ विधिबाट तीव्र रूपमा सन्तानोत्पादन गर्छन्।'
    ],
    examples: ['च्याउ (Agaricus)', 'म्युकर (Mucor)', 'राइजोपस (Rhizopus)', 'यिस्ट (Yeast)', 'पेनिसिलियम (Penicillium)'],
    specimens: ['mucor', 'agaricus', 'yeast']
  },
  plantae: {
    id: 'plantae',
    name: '४. जगत्: प्लान्ती (Kingdom: Plantae)',
    sub: 'Autotrophic Producers',
    badge: 'स्वपोषी वनस्पति',
    badgeColor: 'bg-emerald-500/20 text-emerald-300 border-emerald-500/30',
    type: 'युकारियोटिक (Eukaryotic)',
    organization: 'बहुकोषीय (तन्तु र अङ्ग प्रणाली विकसित)',
    wall: 'सेलुलोज (Cellulose) ले बनेको कोष भित्ता',
    nutrition: 'स्वपोषी (Autotrophic) - प्रकाश-संश्लेषणबाट स्टार्च उत्पादन गर्ने',
    motility: 'अचल (जराले माटोमा स्थिर रहने)',
    features: [
      'क्लोरोफिल (हरितकण) पाइने भएकाले सूर्यको प्रकाशमा खाना बनाउँछन्।',
      'खाना स्टार्च (Starch) को रूपमा सञ्चित हुन्छ।',
      'यसमा लेउ (Algae), उनिउँ (Fern), सल्ला (Gymnosperms) र फूल फुल्ने बिरुवा (Angiosperms) पर्छन्।'
    ],
    examples: ['तोरी (Brassica campestris)', 'उनिउँ (Dryopteris)', 'मकै (Zea mays)', 'धान (Oryza sativa)'],
    specimens: ['mustard', 'fern']
  },
  animalia: {
    id: 'animalia',
    name: '५. जगत्: एनिमेलिया (Kingdom: Animalia)',
    sub: 'Heterotrophic Consumers',
    badge: 'परपोषी जन्तुहरू',
    badgeColor: 'bg-rose-500/20 text-rose-300 border-rose-500/30',
    type: 'युकारियोटिक (Eukaryotic)',
    organization: 'बहुकोषीय (विकसित तन्तु, अङ्ग र स्नायु प्रणाली)',
    wall: 'हुँदैन (Cell wall absent)',
    nutrition: 'परपोषी (Holozoic) - ठोस खाना खाएर शरीरभित्र पाचन गर्ने',
    motility: 'चल (Motile) - प्रायः सबैमा हिँडडुल गर्ने विशेष अङ्ग हुन्छ',
    features: [
      'कोष भित्ता र हरितकण दुवै हुँदैनन्।',
      'विकसित स्नायु प्रणाली र ज्ञानेन्द्रियहरू हुन्छन्।',
      'सञ्चित खाना ग्लाइकोजेन र बोसोको रूपमा रहन्छ।',
      'ढाड नभएका (Invertebrates) र ढाड भएका (Vertebrates) जन्तुहरू पर्दछन्।'
    ],
    examples: ['मानिस (Homo sapiens)', 'बाघ (Panthera tigris)', 'भ्यागुतो (Rana tigrina)', 'गड्यौला (Pheretima)'],
    specimens: ['frog', 'tiger']
  }
};

const C9U2_SPECIMEN_DATA = {
  bacteria: {
    id: 'bacteria',
    name: 'ब्याक्टेरिया (Bacteria)',
    kingdom: 'मोनेरा (Kingdom: Monera)',
    scientificName: 'Eubacteria (e.g. Azotobacter / Rhizobium)',
    icon: '🦠',
    structure: 'एककोषीय प्रोकारियोटिक, कोष भित्ता पेप्टिडोग्लाइकनले बनेको, न्युक्लियोइड र फ्लाजेला।',
    nutrition: 'मृतोपजीवी वा परजीवी (केही नाइट्रोजन स्थिरिकरण गर्ने)।',
    highlight: 'पृथ्वीको सबैभन्दा प्राचीन र सरल प्रोकारियोटिक जीव।'
  },
  nostoc: {
    id: 'nostoc',
    name: 'नोस्टक (Nostoc / Cyanobacteria)',
    kingdom: 'मोनेरा (Kingdom: Monera)',
    scientificName: 'Nostoc commune',
    icon: '🧬',
    structure: 'फिलामेन्टस (माला जस्तो धागो), हेटेरोसिस्ट (Heterocyst) कोष भएको, हरितकणयुक्त।',
    nutrition: 'स्वपोषी (प्रकाश-संश्लेषण गर्ने) र नाइट्रोजन स्थिरिकरण गर्ने।',
    highlight: 'वातावरणीय नाइट्रोजन सोसेर माटोलाई मलिलो बनाउने निलो-हरियो लेउ।'
  },
  amoeba: {
    id: 'amoeba',
    name: 'अमिबा (Amoeba)',
    kingdom: 'प्रोटिस्टा (Kingdom: Protista)',
    scientificName: 'Amoeba proteus',
    icon: '💧',
    structure: 'अनियमित आकार, स्पष्ट न्युक्लियस, नक्कली खुट्टा (Pseudopodia), संकुचित भ्याक्युल।',
    nutrition: 'होलोजोइक (Fagocytosis विधिद्वारा खाना निल्ने)।',
    highlight: 'नक्कली खुट्टाको मद्दतले हिँड्ने र खाना समात्ने एककोषीय युकारियोट।'
  },
  euglena: {
    id: 'euglena',
    name: 'युग्लिना (Euglena)',
    kingdom: 'प्रोटिस्टा (Kingdom: Protista)',
    scientificName: 'Euglena viridis',
    icon: '🪱',
    structure: 'कोर्रा जस्तो फ्लाजेला (Flagella), हरितकण (Chloroplast), आँखाको थोप्लो (Eyespot)।',
    nutrition: 'मिश्रितपोषी (घाममा स्वपोषी, अँध्यारोमा परपोषी)।',
    highlight: 'बिरुवा र जनावर दुवैको साझा कडी (Connecting link)।'
  },
  paramecium: {
    id: 'paramecium',
    name: 'पारामेसियम (Paramecium)',
    kingdom: 'प्रोटिस्टा (Kingdom: Protista)',
    scientificName: 'Paramecium caudatum',
    icon: '🥿',
    structure: 'चप्पल आकार (Slipper shaped), शरीरभरि सिलिया (Cilia), दुईओटा न्युक्लियस (Macronucleus र Micronucleus)।',
    nutrition: 'होलोजोइक (ओरल ग्रुभबाट ब्याक्टेरिया खाने)।',
    highlight: 'सिलियाको तीव्र कम्पनबाट पानीमा पौडने विकसित प्रोटिस्टा।'
  },
  mucor: {
    id: 'mucor',
    name: 'म्युकर / रोटीको ढुसी (Mucor)',
    kingdom: 'फन्जाई (Kingdom: Fungi)',
    scientificName: 'Mucor mucedo',
    icon: '🍞',
    structure: 'माइसेलियम, हाइफी, ठाडो स्पोरेन्जियोफोर र टुप्पोमा कालो बीजाणुधानी (Sporangium)।',
    nutrition: 'स्याप्रोट्रोफिक (रोटीको जैविक पदार्थ सोस्ने)।',
    highlight: 'ओसिलो रोटीमा सेतो कपास जस्तो भुवादार रूपमा तीव्र फैलिने ढुसी।'
  },
  agaricus: {
    id: 'agaricus',
    name: 'च्याउ (Mushroom / Agaricus)',
    kingdom: 'फन्जाई (Kingdom: Fungi)',
    scientificName: 'Agaricus bisporus / campestris',
    icon: '🍄',
    structure: 'माइसेलियम (माटोमुनि), ठाडो डाँठ (Stipe), छाता (Pileus), र गिल्स (Gills)।',
    nutrition: 'मृतोपजीवी (गोबर, पराल र सडेको माटोबाट सोस्ने)।',
    highlight: 'गिल्सभित्र लाखौँ ब्यासिडियोस्पोर बनाई फैलने पौष्टिक बहुकोषीय ढुसी।'
  },
  yeast: {
    id: 'yeast',
    name: 'यिस्ट (Yeast)',
    ...
    icon: '🥯',
    structure: 'एककोषीय गोलाकार वा अण्डाकार, माइसेलियम नभएको, बडिङ (Budding) हुने।',
    nutrition: 'मृतोपजीवी (चिनीलाई अल्कोहल र CO2 मा बदल्ने)।',
    highlight: 'बेकिङ उद्योग (पाउरोटी फुलाउन) र रक्सी बनाउन प्रयोग हुने मित्रजीव।'
  },
  mustard: {
    id: 'mustard',
    name: 'तोरी (Mustard)',
    kingdom: 'प्लान्ती (Kingdom: Plantae)',
    scientificName: 'Brassica campestris',
    icon: '🌻',
    structure: 'जरा, काण्ड, पात, पहेँलो फूल र दुई दलीय बीउ भएको फूल फुल्ने बिरुवा।',
    nutrition: 'स्वपोषी (हरितकणबाट प्रकाश-संश्लेषण गर्ने)।',
    highlight: 'नेपालमा तेलहन बालीको रूपमा प्रख्यात वार्षिक बिरुवा।'
  },
  fern: {
    id: 'fern',
    name: 'उनिउँ (Fern)',
    ...
    icon: '🌿',
    structure: 'संवहन तन्तु (Xylem & Phloem) भएको, पातको पछाडि सोराई (Sori) मा बीजाणु हुने।',
    nutrition: 'स्वपोषी (प्रकाश-संश्लेषक)।',
    highlight: 'फूल नफुल्ने तर संवहन नली विकसित भएको टेरिडोफाइटा बिरुवा।'
  },
  frog: {
    id: 'frog',
    name: 'भ्यागुतो (Frog)',
    kingdom: 'एनिमेलिया (Kingdom: Animalia)',
    scientificName: 'Rana tigrina',
    icon: '🐸',
    structure: 'ढाड भएको (Vertebrate), ओसिलो छाला, पछिल्लो खुट्टा लामो र फड्को मार्ने।',
    nutrition: 'होलोजोइक (कीरा-फट्याङ्ग्रा सिकार गर्ने मांसाहारी)।',
    highlight: 'पानी र जमिन दुवैमा बस्न सक्ने उभयचर (Amphibian) ढाड भएको जन्तु।'
  },
  tiger: {
    id: 'tiger',
    name: 'पाटे बाघ (Royal Bengal Tiger)',
    kingdom: 'एनिमेलिया (Kingdom: Animalia)',
    scientificName: 'Panthera tigris',
    icon: '🐅',
    structure: 'विकसित मस्तिष्क, बलियो बङ्गारा, पहेँलो पृष्ठभूमिमा कालो पाटा भएको स्तनधारी।',
    nutrition: 'होलोजोइक (मांसाहारी सिकारी)।',
    highlight: 'वनको पारिस्थितिक प्रणालीको सर्वोच्च उपभोक्ता।'
  }
};

function selectKingdomC9U2(kId) {
  activeC9U2Kingdom = kId;
  const data = C9U2_KINGDOM_DATA[kId];
  if (!data) return;

  // Update kingdom buttons
  const kIds = ['monera', 'protista', 'fungi', 'plantae', 'animalia'];
  kIds.forEach(id => {
    const btn = document.getElementById(`c9u2-kbtn-${id}`);
    if (btn) {
      if (id === kId) {
        btn.className = 'px-4 py-2 rounded-2xl text-xs font-black transition border bg-cyan-500/20 border-cyan-400 text-cyan-300 shadow-sm cursor-pointer';
      } else {
        btn.className = 'px-4 py-2 rounded-2xl text-xs font-black transition border bg-slate-800/80 border-slate-700 text-slate-400 hover:text-white cursor-pointer';
      }
    }
  });

  // Render Kingdom Details Panel
  const panel = document.getElementById('c9u2-kingdom-panel');
  if (panel) {
    panel.innerHTML = `
      <div class="flex flex-wrap items-center justify-between gap-3 border-b border-slate-800 pb-3">
        <div>
          <span class="px-2.5 py-0.5 rounded-full text-[10px] font-black uppercase tracking-wider border ${data.badgeColor}">
            ${data.badge}
          </span>
          <h4 class="text-base md:text-lg font-black text-white mt-1">${data.name}</h4>
          <span class="text-xs text-slate-400 font-mono">${data.sub}</span>
        </div>
        <div class="text-xs text-slate-400">
          उदाहरणहरू: <span class="text-cyan-300 font-bold">${data.examples.join(', ')}</span>
        </div>
      </div>

      <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-3 text-xs">
        <div class="p-3 rounded-xl bg-slate-900 border border-slate-800">
          <div class="text-slate-400 font-medium mb-1">कोषको प्रकार (Cell Type)</div>
          <div class="text-cyan-300 font-bold">${data.type}</div>
        </div>
        <div class="p-3 rounded-xl bg-slate-900 border border-slate-800">
          <div class="text-slate-400 font-medium mb-1">संगठन (Organization)</div>
          <div class="text-emerald-300 font-bold">${data.organization}</div>
        </div>
        <div class="p-3 rounded-xl bg-slate-900 border border-slate-800">
          <div class="text-slate-400 font-medium mb-1">कोष भित्ता (Cell Wall)</div>
          <div class="text-amber-300 font-bold">${data.wall}</div>
        </div>
        <div class="p-3 rounded-xl bg-slate-900 border border-slate-800">
          <div class="text-slate-400 font-medium mb-1">पोषण विधि (Nutrition)</div>
          <div class="text-purple-300 font-bold">${data.nutrition}</div>
        </div>
      </div>

      <div class="bg-slate-900/90 p-3.5 rounded-xl border border-slate-800 text-xs space-y-1.5">
        <div class="font-bold text-slate-300">🌟 यस जगत्का प्रमुख विशेषताहरू:</div>
  ...
  renderSpecimenButtons();

  // Select first representative specimen if current specimen not in this kingdom
  if (!data.specimens.includes(activeC9U2Specimen)) {
    selectSpecimenC9U2(data.specimens[0]);
  } else {
    selectSpecimenC9U2(activeC9U2Specimen);
  }
}

function renderSpecimenButtons() {
  const container = document.getElementById('c9u2-specimen-buttons');
  if (!container) return;
  const specimens = Object.values(C9U2_SPECIMEN_DATA);
  container.innerHTML = specimens.map(s => {
    const isSelected = s.id === activeC9U2Specimen;
    const isCurrentKingdom = C9U2_KINGDOM_DATA[activeC9U2Kingdom]?.specimens.includes(s.id);
    const borderCls = isSelected ? 'border-cyan-400 bg-cyan-950/60 text-cyan-200' : (isCurrentKingdom ? 'border-slate-700 bg-slate-900 text-slate-300 hover:border-slate-500' : 'border-slate-800/60 bg-slate-950/40 text-slate-500 hover:text-slate-300');
    return `
      <button onclick="selectSpecimenC9U2('${s.id}')" class="p-2.5 rounded-xl border ${borderCls} text-left text-xs transition flex items-center gap-2 cursor-pointer">
        <span class="text-lg">${s.icon}</span>
        <span class="truncate font-semibold">${s.name.split(' ')[0]}</span>
      </button>
    `;
  }).join('');
}

function selectSpecimenC9U2(sId) {
  activeC9U2Specimen = sId;
  const s = C9U2_SPECIMEN_DATA[sId];
  if (!s) return;

  const lbl = document.getElementById('c9u2-selected-specimen-label');
  if (lbl) lbl.textContent = s.scientificName;

  renderSpecimenButtons();

  const display = document.getElementById('c9u2-specimen-display');
  if (display) {
    display.innerHTML = `
      <div class="flex flex-wrap items-center justify-between gap-4 border-b border-slate-800 pb-3">
        <div class="flex items-center gap-3">
          <div class="w-12 h-12 rounded-2xl bg-cyan-500/20 border border-cyan-500/30 flex items-center justify-center text-3xl">
            ${s.icon}
          </div>
          <div>
            <h4 class="text-base md:text-lg font-black text-white">${s.name}</h4>
            <div class="text-xs text-cyan-400 font-mono italic">${s.scientificName}</div>
          </div>
        </div>
      ...
      <div class="grid grid-cols-1 md:grid-cols-3 gap-4 text-xs">
        <div class="p-3.5 rounded-xl bg-slate-900/90 border border-slate-800 space-y-1">
          <div class="text-slate-400 font-bold">🔬 सूक्ष्म संरचना (Microscopic Structure):</div>
          <div class="text-slate-300">${s.structure}</div>
        </div>
        <div class="p-3.5 rounded-xl bg-slate-900/90 border border-slate-800 space-y-1">
          <div class="text-slate-400 font-bold">🍽️ पोषण विधि (Mode of Nutrition):</div>
          <div class="text-slate-300">${s.nutrition}</div>
        </div>
        <div class="p-3.5 rounded-xl bg-slate-900/90 border border-slate-800 space-y-1">
          <div class="text-slate-400 font-bold">✨ मुख्य वैज्ञानिक पहिचान:</div>
          <div class="text-cyan-300 font-medium">${s.highlight}</div>
        </div>
      </div>
    `;
  }
...
// ================= 2. BINOMIAL NOMENCLATURE STUDIO DATA & ENGINE =================
const C9U2_BINOMIAL_DB = {
  human: {
    nepali: 'मानिस',
    english: 'Modern Human',
    genus: 'Homo',
    species: 'sapiens',
    family: 'Hominidae',
    order: 'Primates',
    ...
    phylum: 'Chordata',
    kingdom: 'Animalia',
    meaning: 'Homo = ल्याटिनमा "मानव" वा "पृथ्वीवासी", sapiens = "बुद्धिमान वा ज्ञानी"'
  },
  mustard: {
    nepali: 'तोरी',
    ...
    family: 'Brassicaceae',
    order: 'Brassicales',
    classRank: 'Dicotyledonae',
    phylum: 'Tracheophyta',
    kingdom: 'Plantae',
    meaning: 'Brassica = बन्दागोभी/तोरी समूहको वनस्पति, campestris = "खुला खेतबारीमा उम्रने"'
  },
  pea: {
    nepali: 'केराउ',
    english: 'Garden Pea',
    ...
    phylum: 'Tracheophyta',
    kingdom: 'Plantae',
    meaning: 'Pisum = केराउ, sativum = "खेतबारीमा खेती गरिने"'
  },
  leopard: {
    nepali: 'चितुवा',
    english: 'Leopard',
    ...
    phylum: 'Chordata',
    kingdom: 'Animalia',
    meaning: 'Panthera = गर्जने ठूला बिरालाहरू, pardus = "चितुवा वा छिरबिरे"'
  },
  tiger: {
    nepali: 'बाघ',
    ...
    phylum: 'Chordata',
    kingdom: 'Animalia',
    meaning: 'Panthera = ठूला मांसाहारी बिराला, tigris = "तीव्र वेग भएको बाण जस्तो"'
  },
  lion: {
    nepali: 'सिंह',
    english: 'Lion',
    ...
    phylum: 'Chordata',
    kingdom: 'Animalia',
    meaning: 'Panthera = ठूला बिरालाको जेनस, leo = "सिंह"'
  },
  cat: {
    nepali: 'बिरालो',
    ...
    phylum: 'Chordata',
    kingdom: 'Animalia',
    meaning: 'Felis = साना बिरालाको समूह, catus = "घरपालुवा बिरालो"'
  },
  dog: {
    nepali: 'कुकुर',
    english: 'Domestic Dog / Wolf',
    genus: 'Canis',
    species: 'lupus',
    family: 'Canidae',
    order: 'Carnivora',
    ...
    phylum: 'Chordata',
    kingdom: 'Animalia',
    meaning: 'Canis = कुकुर परिवार, lupus = "ब्वाँसो/कुकुर"'
  },
  cow: {
    nepali: 'गाई',
    english: 'Cow / Cattle',
    ...
    phylum: 'Chordata',
    kingdom: 'Animalia',
    meaning: 'Bos = गोरु/गाई प्रजाति, taurus = "साँढे/गाई"'
  },
  potato: {
    nepali: 'आलु',
    english: 'Potato',
    ...
    phylum: 'Tracheophyta',
    kingdom: 'Plantae',
    meaning: 'Solanum = भन्टा-आलु परिवार, tuberosum = "जरामा ट्युबर (गाँठो) लाग्ने"'
  },
  paddy: {
    nepali: 'धान',
    english: 'Asian Rice',
    ...
    family: 'Poaceae',
    order: 'Poales',
    classRank: 'Monocotyledonae',
    phylum: 'Tracheophyta',
    kingdom: 'Plantae',
    meaning: 'Oryza = धान/चामल, sativa = "खेती गरिने अन्न"'
  },
  maize: {
    nepali: 'मकै',
    english: 'Corn / Maize',
    ...
    phylum: 'Tracheophyta',
    kingdom: 'Plantae',
    meaning: 'Zea = जीवन दिने अन्न, mays = "मकैको स्थानीय नाम"'
  }
};

function selectBinomialOrganism(orgKey) {
  const org = C9U2_BINOMIAL_DB[orgKey];
  if (!org) return;

  const card = document.getElementById('c9u2-binomial-detail-card');
  if (card) {
    card.innerHTML = `
      <div class="flex items-center justify-between border-b border-slate-800 pb-2">
        <span class="text-sm font-bold text-white">${org.nepali} (${org.english})</span>
        <span class="text-xs bg-cyan-600/30 text-cyan-300 border border-cyan-500/40 px-2.5 py-0.5 rounded-full font-mono">
          <em>${org.genus} ${org.species}</em>
        </span>
      </div>

      <div class="grid grid-cols-2 gap-2 text-xs">
        <div class="p-2.5 rounded-lg bg-slate-950 border border-slate-800">
          <div class="text-slate-400 text-[10px]">१. पहिलो पद (Generic Name)</div>
          <div class="font-bold text-cyan-300 text-sm font-mono">${org.genus}</div>
          <div class="text-[10px] text-emerald-400 mt-0.5">✓ पहिलो अक्षर Capital</div>
        </div>
        <div class="p-2.5 rounded-lg bg-slate-950 border border-slate-800">
          <div class="text-slate-400 text-[10px]">२. दोस्रो पद (Specific Name)</div>
          <div class="font-bold text-amber-300 text-sm font-mono">${org.species}</div>
          <div class="text-[10px] text-emerald-400 mt-0.5">✓ सबै अक्षर Small letter</div>
        </div>
      </div>

      <div class="p-2.5 rounded-lg bg-slate-950/80 border border-slate-800 text-xs text-slate-300 space-y-1">
        <div class="text-slate-400 font-semibold text-[11px]">शब्दको ल्याटिन अर्थ:</div>
        <div class="italic text-cyan-200">${org.meaning}</div>
      </div>

      <div class="p-2.5 rounded-lg bg-slate-950/80 border border-slate-800 text-[11px] font-mono text-slate-400 space-y-0.5">
        <div class="text-slate-300 font-bold mb-1">वर्गीकरण तह (Taxonomic Ranks):</div>
        <div>Kingdom: <span class="text-white">${org.kingdom}</span></div>
        <div>Phylum/Div: <span class="text-white">${org.phylum}</span></div>
        <div>Class: <span class="text-white">${org.classRank}</span></div>
        <div>Order: <span class="text-white">${org.order}</span></div>
        <div>Family: <span class="text-white">${org.family}</span></div>
        <div>Genus: <span class="text-cyan-300 font-bold">${org.genus}</span></div>
        <div>Species: <span class="text-amber-300 font-bold">${org.species}</span></div>
      </div>
    `;
  }
}

function buildCustomScientificName() {
  const common = document.getElementById('c9u2-input-common')?.value.trim() || 'अज्ञात जीव';
  let genus = document.getElementById('c9u2-input-genus')?.value.trim() || '';
  let species = document.getElementById('c9u2-input-species')?.value.trim() || '';
  const isItalic = document.getElementById('c9u2-check-italic')?.checked ?? true;

  const preview = document.getElementById('c9u2-custom-name-preview');
  if (!preview) return;
  ...
  // Check Genus rules
  if (!genus) {
    warnings.push('जेनस (Generic Name) खाली छ।');
  } else if (genus[0] !== genus[0].toUpperCase()) {
    warnings.push(`जेनस '${genus}' को पहिलो अक्षर ठूलो (Capital) हुनुपर्छ → सुझाव: '${genus[0].toUpperCase() + genus.slice(1)}'`);
  } else {
    passes.push(`जेनस '${genus}': पहिलो अक्षर Capital नियम पालना भयो।`);
  }

  // Check Species rules
  if (!species) {
    warnings.push('स्पेसिस (Specific Name) खाली छ।');
  } else if (species !== species.toLowerCase()) {
    warnings.push(`स्पेसिस '${species}' को सबै अक्षर सानो (Small) हुनुपर्छ → सुझाव: '${species.toLowerCase()}'`);
  } else {
    passes.push(`स्पेसिस '${species}': साना अक्षरहरूको नियम पालना भयो।`);
  }

  // Formatting display
  const formattedName = `${genus || '___'} ${species || '___'}`;
  const fontStyle = isItalic ? 'italic' : 'underline';

  preview.innerHTML = `
    <div class="border-b border-slate-800 pb-2">
      <div class="text-[11px] text-slate-400 font-semibold">नतिजा पूर्वावलोकन (Preview):</div>
      <div class="text-base font-bold text-white mt-1">
        ${common}: <span class="text-cyan-400 font-mono ${fontStyle}">${formattedName}</span>
      </div>
    </div>
    ...
    <div class="space-y-1.5 text-xs">
      ...
      ${warnings.length === 0 ? `
        <div class="p-2 rounded-lg bg-emerald-500/10 border border-emerald-500/20 text-emerald-300 text-xs font-bold text-center mt-2">
          🎉 क्यारोलस लिनियसको दुईपदीय नामाकरणका सम्पूर्ण नियमहरू १००% मिलेका छन्!
        </div>
      ` : ''}
    </div>
  `;
...
// ================= 3. DICHOTOMOUS KEY LAB DATA & ENGINE =================
const C9U2_DICHOTOMOUS_DATA = {
  bacteria: {
    name: 'नमुना १ (Bacteria)',
    icon: '🦠',
    answers: { step1: 'no' },
    finalKingdom: 'मोनेरा (Kingdom: Monera)',
    finalReason: 'कोषमा स्पष्ट न्युक्लियस नभएको प्रोकारियोटिक एककोषीय जीव।'
  },
  amoeba: {
    name: 'नमुना २ (Amoeba)',
    icon: '💧',
    answers: { step1: 'yes', step2: 'unicellular' },
    finalKingdom: 'प्रोटिस्टा (Kingdom: Protista)',
    finalReason: 'स्पष्ट न्युक्लियस भएको एककोषीय युकारियोटिक जीव।'
  },
  mucor: {
    name: 'नमुना ३ (Mucor)',
    icon: '🍄',
    answers: { step1: 'yes', step2: 'multicellular', step3: 'no', step4: 'sapro' },
    finalKingdom: 'फन्जाई (Kingdom: Fungi)',
    finalReason: 'हरितकण नभएको, काइटिनको कोष भित्ता भएको बहुकोषीय मृतोपजीवी जीव।'
  },
  mustard: {
    name: 'नमुना ४ (Mustard)',
    icon: '🌻',
    answers: { step1: 'yes', step2: 'multicellular', step3: 'yes' },
    finalKingdom: 'प्लान्ती (Kingdom: Plantae)',
    finalReason: 'सेलुलोजको कोष भित्ता र हरितकण भएको बहुकोषीय स्वपोषी वनस्पति।'
  },
  tiger: {
    name: 'नमुना ५ (Tiger)',
    icon: '🐅',
    answers: { step1: 'yes', step2: 'multicellular', step3: 'no', step4: 'holo' },
    finalKingdom: 'एनिमेलिया (Kingdom: Animalia)',
    finalReason: 'कोष भित्ता र हरितकण नभएको, ठोस खाना खाने बहुकोषीय परपोषी जन्तु।'
  }
};

function startDichotomousKey(specimenId) {
  activeC9U2DichotomousSpecimen = specimenId;
  activeC9U2DichotomousStep = 1;

  const specIds = ['bacteria', 'amoeba', 'mucor', 'mustard', 'tiger'];
  specIds.forEach(id => {
    const btn = document.getElementById(`c9u2-kpick-${id}`);
    if (btn) {
      if (id === specimenId) {
        btn.className = 'px-3 py-2 rounded-xl border bg-cyan-600/30 border-cyan-500 text-cyan-300 text-center transition cursor-pointer font-bold';
      } else {
        btn.className = 'px-3 py-2 rounded-xl border bg-slate-900 border-slate-800 text-slate-400 hover:text-white text-center transition cursor-pointer font-bold';
      }
    }
  });
...
function resetDichotomousKey() {
  startDichotomousKey('bacteria');
}

function renderDichotomousStep() {
  const box = document.getElementById('c9u2-dichotomous-step-box');
  if (!box) return;

  const currentSpecimen = C9U2_DICHOTOMOUS_DATA[activeC9U2DichotomousSpecimen];
  const step = activeC9U2DichotomousStep;

  if (step === 1) {
    box.innerHTML = `
      <div class="space-y-4">
        <div class="flex items-center justify-between border-b border-slate-800 pb-2">
          <span class="text-xs text-slate-400 font-bold">चरण १ (Step 1): कोषको न्युक्लियस जाँच</span>
          <span class="text-xs text-cyan-400 font-bold">${currentSpecimen.icon} ${currentSpecimen.name}</span>
        </div>
        <div class="text-sm font-bold text-white">
          १. के यस सजीवको कोषमा न्युक्लियर झिल्लीले घेरिएको स्पष्ट न्युक्लियस (Definite Nucleus) छ ?
        </div>
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-3 text-xs">
          <button onclick="answerDichotomousKey('no')" class="p-3 rounded-xl bg-slate-800 hover:bg-slate-700 border border-slate-700 text-white font-bold text-left transition cursor-pointer flex items-center justify-between">
            <span>(क) छैन (अविकाशित न्युक्लियोइड मात्र छ)</span>
            <span class="text-cyan-400">→ मोनेरा जगत्</span>
          </button>
          <button onclick="answerDichotomousKey('yes')" class="p-3 rounded-xl bg-slate-800 hover:bg-slate-700 border border-slate-700 text-white font-bold text-left transition cursor-pointer flex items-center justify-between">
            <span>(ख) छ (स्पष्ट विकसित न्युक्लियस छ)</span>
            <span class="text-cyan-400">→ चरण २ मा जानुहोस्</span>
          </button>
        </div>
      </div>
    `;
    ...
    box.innerHTML = `
      <div class="space-y-4">
        <div class="flex items-center justify-between border-b border-slate-800 pb-2">
          <span class="text-xs text-slate-400 font-bold">चरण २ (Step 2): कोषीय संगठन जाँच</span>
          <span class="text-xs text-cyan-400 font-bold">${currentSpecimen.icon} ${currentSpecimen.name}</span>
        </div>
        <div class="text-sm font-bold text-white">
          २. यो सजीव एककोषीय (Unicellular) हो कि बहुकोषीय (Multicellular) हो ?
        </div>
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-3 text-xs">
          <button onclick="answerDichotomousKey('unicellular')" class="p-3 rounded-xl bg-slate-800 hover:bg-slate-700 border border-slate-700 text-white font-bold text-left transition cursor-pointer flex items-center justify-between">
            <span>(क) एककोषीय (एउटा मात्र कोषले बनेको)</span>
            <span class="text-teal-400">→ प्रोटिस्टा जगत्</span>
          </button>
          <button onclick="answerDichotomousKey('multicellular')" class="p-3 rounded-xl bg-slate-800 hover:bg-slate-700 border border-slate-700 text-white font-bold text-left transition cursor-pointer flex items-center justify-between">
            <span>(ख) बहुकोषीय (धेरै कोषिकाहरूले बनेको)</span>
            <span class="text-teal-400">→ चरण ३ मा जानुहोस्</span>
          </button>
        </div>
      </div>
    `;
    ...
    box.innerHTML = `
      <div class="space-y-4">
        <div class="flex items-center justify-between border-b border-slate-800 pb-2">
          <span class="text-xs text-slate-400 font-bold">चरण ३ (Step 3): प्रकाश-संश्लेषण र हरितकण जाँच</span>
          <span class="text-xs text-cyan-400 font-bold">${currentSpecimen.icon} ${currentSpecimen.name}</span>
        </div>
        <div class="text-sm font-bold text-white">
          ३. के यस सजीवको कोषमा हरितकण (Chlorophyll) पाइन्छ र प्रकाश-संश्लेषण गर्छ ?
        </div>
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-3 text-xs">
          <button onclick="answerDichotomousKey('yes')" class="p-3 rounded-xl bg-slate-800 hover:bg-slate-700 border border-slate-700 text-white font-bold text-left transition cursor-pointer flex items-center justify-between">
            <span>(क) गर्छ (सेलुलोजको कोष भित्ता र स्वपोषी)</span>
            <span class="text-emerald-400">→ प्लान्ती जगत्</span>
          </button>
          <button onclick="answerDichotomousKey('no')" class="p-3 rounded-xl bg-slate-800 hover:bg-slate-700 border border-slate-700 text-white font-bold text-left transition cursor-pointer flex items-center justify-between">
            <span>(ख) गर्दैन (हरितकणविहीन र परपोषी)</span>
            <span class="text-emerald-400">→ चरण ४ मा जानुहोस्</span>
          </button>
        </div>
      </div>
    `;
    ...
    box.innerHTML = `
      <div class="space-y-4">
        <div class="flex items-center justify-between border-b border-slate-800 pb-2">
          <span class="text-xs text-slate-400 font-bold">चरण ४ (Step 4): कोष भित्ता र पोषण विधि</span>
          <span class="text-xs text-cyan-400 font-bold">${currentSpecimen.icon} ${currentSpecimen.name}</span>
        </div>
        <div class="text-sm font-bold text-white">
          ४. यस सजीवमा कोष भित्ता र पोषण विधि कस्तो छ ?
        </div>
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-3 text-xs">
          <button onclick="answerDichotomousKey('sapro')" class="p-3 rounded-xl bg-slate-800 hover:bg-slate-700 border border-slate-700 text-white font-bold text-left transition cursor-pointer flex items-center justify-between">
            <span>(क) काइटिनको कोष भित्ता छ, सडेगलेकोबाट सोस्छ</span>
            <span class="text-amber-400">→ फन्जाई जगत्</span>
          </button>
          <button onclick="answerDichotomousKey('holo')" class="p-3 rounded-xl bg-slate-800 hover:bg-slate-700 border border-slate-700 text-white font-bold text-left transition cursor-pointer flex items-center justify-between">
            <span>(ख) कोष भित्ता छैन, ठोस खाना निलेर खान्छ</span>
            <span class="text-rose-400">→ एनिमेलिया जगत्</span>
          </button>
        </div>
      </div>
    `;
    ...
    // Finished state
    box.innerHTML = `
      <div class="p-4 rounded-2xl bg-gradient-to-br from-cyan-950 to-slate-900 border border-cyan-500/40 text-center space-y-3">
        <div class="text-4xl">${currentSpecimen.icon}</div>
        <h4 class="text-lg font-black text-white">
          पहिचान सफल भयो: <span class="text-cyan-400">${currentSpecimen.finalKingdom}</span>
        </h4>
        <p class="text-xs text-slate-300 max-w-md mx-auto leading-relaxed">
          ${currentSpecimen.finalReason}
        </p>
        <div class="pt-2">
          <button onclick="resetDichotomousKey()" class="px-4 py-2 rounded-xl bg-cyan-600 hover:bg-cyan-500 text-white font-bold text-xs transition cursor-pointer">
            अर्को नमुना पहिचान गर्नुहोस्
          </button>
        </div>
      </div>
    `;
...
function answerDichotomousKey(choice) {
  const expectedAnswers = C9U2_DICHOTOMOUS_DATA[activeC9U2DichotomousSpecimen].answers;
  const currentStepKey = `step${activeC9U2DichotomousStep}`;

  if (activeC9U2DichotomousStep === 1) {
    if (choice === 'no') {
      activeC9U2DichotomousStep = 99; // Monera identified
    } else {
      activeC9U2DichotomousStep = 2;
    }
  } else if (activeC9U2DichotomousStep === 2) {
    if (choice === 'unicellular') {
      activeC9U2DichotomousStep = 99; // Protista identified
    } else {
      activeC9U2DichotomousStep = 3;
    }
  } else if (activeC9U2DichotomousStep === 3) {
    if (choice === 'yes') {
      activeC9U2DichotomousStep = 99; // Plantae identified
    } else {
      activeC9U2DichotomousStep = 4;
    }
  } else if (activeC9U2DichotomousStep === 4) {
    activeC9U2DichotomousStep = 99; // Fungi or Animalia identified
  }
...
// ================= 4. EXERCISES FILTER =================
function filterC9U2Exercises(cat) {
  const cards = document.querySelectorAll('.c9u2-ex-card');
  cards.forEach(card => {
  ...
  const catBtns = ['all', 'mcq', 'reason', 'diff', 'qa', 'project'];
  catBtns.forEach(c => {
    const btn = document.getElementById(`c9u2-exbtn-${c}`);
    if (btn) {
      if (c === cat) {
        btn.className = 'px-3 py-1.5 rounded-xl text-xs font-black transition bg-cyan-600 text-white shadow-sm cursor-pointer whitespace-nowrap';
      } else {
...
// ================= 5. TIERS FILTER =================
function filterC9U2Tiers(tier) {
  const cards = document.querySelectorAll('.c9u2-tier-card');
  cards.forEach(card => {
  ...
  tierBtns.forEach(t => {
    const btn = document.getElementById(`c9u2-tierbtn-${t}`);
    if (btn) {
      if (t === tier) {
        btn.className = 'px-3 py-1.5 rounded-xl text-xs font-black transition bg-cyan-600 text-white shadow-sm cursor-pointer whitespace-nowrap';
      } else {
...
// ================= 6. SELF-ASSESSMENT QUIZ ENGINE =================
const C9U2_QUIZ_DATA = [
  {
    q: '१. ब्लुग्रिन एल्गी (Cyanobacteria / Nostoc) कुन जगत्अन्तर्गत पर्दछ ?',
    options: ['मोनेरा (Monera)', 'प्रोटिस्टा (Protista)', 'फन्जाई (Fungi)', 'प्लान्ती (Plantae)'],
    correct: 0,
    explanation: 'ब्लुग्रिन एल्गी एककोषीय र स्पष्ट न्युक्लियस नभएको प्रोकारियोटिक सूक्ष्मजीव भएकाले यो मोनेरा जगत्मा पर्दछ।'
  },
  {
    q: '२. आधुनिक ट्याक्सोनोमीका पिता (Father of Modern Taxonomy) कसलाई मानिन्छ ?',
    options: ['आर. एच. ह्विट्टेकर', 'क्यारोलस लिनियस', 'रोबर्ट हुक', 'चार्ल्स डार्विन'],
    correct: 1,
    explanation: 'क्यारोलस लिनियसले वैज्ञानिक वर्गीकरण र दुईपदीय नामाकरण पद्धति प्रतिपादन गरेकाले उनलाई Father of Modern Taxonomy भनिन्छ।'
  },
  {
    q: '३. सजीवहरूको पाँच जगत् वर्गीकरण प्रणाली कसले कहिले प्रतिपादन गरेका हुन् ?',
    options: ['Carolus Linnaeus (1753)', 'Robert H. Whittaker (1969)', 'Robert Koch (1882)', 'Chatton (1925)'],
    correct: 1,
    explanation: 'अमेरिकी वैज्ञानिक आर. एच. ह्विट्टेकर (R. H. Whittaker) ले सन् १९६९ मा पाँच जगत् वर्गीकरण प्रणाली प्रस्तुत गरे।'
  },
  {
    q: '४. ब्याक्टेरियाको कोष भित्ता कुन रासायनिक पदार्थले बनेको हुन्छ ?',
    options: ['सेलुलोज (Cellulose)', 'काइटिन (Chitin)', 'पेप्टिडोग्लाइकन (Peptidoglycan)', 'लिग्निन (Lignin)'],
    correct: 2,
    explanation: 'मोनेरा जगत्का ब्याक्टेरियाहरूको बाहिरी कोष भित्ता पेप्टिडोग्लाइकन (Peptidoglycan) ले बनेको हुन्छ।'
  },
  {
    q: '५. कुन विशेषताका कारणले गर्दा युग्लिनालाई बिरुवा र जनावर दुवैको साझा कडी मानिन्छ ?',
    options: ['यसमा न्युक्लियस नहुनाले', 'यसमा हरितकण र फ्लाजेला दुवै पाइने हुनाले', 'यो बहुकोषीय हुनाले', 'यो हावामा उड्न सक्ने हुनाले'],
    correct: 1,
    explanation: 'युग्लिनामा बिरुवा जस्तै हरितकण (प्रकाश-संश्लेषण गर्न) र जनावर जस्तै फ्लाजेला (पौडन) पाइने हुनाले यसलाई साझा कडी मानिन्छ।'
  },
  {
    q: '६. च्याउ, म्युकर र ढुसीको कोष भित्ता कुन कडा पदार्थले बनेको हुन्छ ?',
    options: ['पेक्टिन', 'सेलुलोज', 'काइटिन (Chitin)', 'पेप्टाइड'],
    correct: 2,
    explanation: 'फन्जाई जगत्का जीवहरूको कोष भित्ता कडा नाइट्रोजनयुक्त पोलिस्याकराइड "काइटिन" ले बनेको हुन्छ।'
  },
  {
    q: '७. तोरीको सही दुईपदीय वैज्ञानिक नाम कुन हो ?',
    options: ['Brassica Campestris', 'brassica campestris', 'Brassica campestris', 'Pisum sativum'],
    correct: 2,
    explanation: 'लिनियन नियम अनुसार जेनसको पहिलो अक्षर Capital र स्पेसिसको पहिलो अक्षर Small हुनुपर्छ: Brassica campestris।'
  },
  {
    q: '८. सजीवहरूको वर्गीकरणको सबैभन्दा तल्लो, प्राकृतिक र आधारभूत एकाइ कुन हो ?',
    options: ['जगत् (Kingdom)', 'जेनस (Genus)', 'फेमिली (Family)', 'स्पेसिस (Species)'],
    correct: 3,
    explanation: 'स्पेसिस (Species / प्रजाति) ट्याक्सोनोमीको सबैभन्दा सानो तथा आधारभूत प्रजननक्षम प्राकृतिक एकाइ हो।'
  },
  {
    q: '९. बेकिङ (पाउरोटी बनाउन) र मदिरा किण्वनमा प्रयोग हुने एककोषीय ढुसी कुन हो ?',
    options: ['अमिबा', 'यिस्ट (Yeast)', 'म्युकर', 'पेनिसिलियम'],
    correct: 1,
    explanation: 'यिस्ट (Yeast) फन्जाई जगत्मा पर्ने एककोषीय ढुसी हो जसले किण्वन (Fermentation) प्रक्रियाबाट खाना फुलाउन र अल्कोहल बनाउन मद्दत गर्छ।'
  },
  {
    q: '१०. ढुसी (Fungi) ले आफ्नो शरीरमा अतिरिक्त खाना कुन रूपमा सञ्चित गर्छ ?',
    options: ['स्टार्च (Starch)', 'ग्लाइकोजेन (Glycogen)', 'ग्लुकोज', 'सेलुलोज'],
    correct: 1,
    explanation: 'बिरुवाले खाना स्टार्चका रूपमा राख्छन् भने ढुसी र जन्तुहरूले खाना ग्लाइकोजेन (Glycogen) का रूपमा सञ्चित गर्छन्।'
  }
];

function renderC9U2Quiz() {
  const container = document.getElementById('c9u2-quiz-container');
  if (!container) return;

  container.innerHTML = C9U2_QUIZ_DATA.map((item, qIdx) => {
    const answered = c9u2QuizAnswers[qIdx] !== undefined;
    const selected = c9u2QuizAnswers[qIdx];
    const isCorrect = selected === item.correct;

    return `
      <div class="bg-white border border-slate-200 rounded-3xl p-5 shadow-sm space-y-3">
        <div class="text-xs md:text-sm font-bold text-slate-900 leading-snug">
          ${item.q}
        </div>
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-2 text-xs">
          ${item.options.map((opt, optIdx) => {
            let btnCls = 'bg-slate-50 hover:bg-slate-100 border-slate-200 text-slate-700';
            let icon = `<span class="w-4 h-4 rounded-full border border-slate-300 flex items-center justify-center text-[10px] text-slate-400 font-mono">${String.fromCharCode(65 + optIdx)}</span>`;
            
            if (answered) {
              if (optIdx === item.correct) {
                btnCls = 'bg-emerald-50 border-emerald-400 text-emerald-900 font-bold';
                icon = `<span class="w-4 h-4 rounded-full bg-emerald-600 text-white flex items-center justify-center text-[10px]">✓</span>`;
              } else if (optIdx === selected) {
                btnCls = 'bg-rose-50 border-rose-400 text-rose-900 font-bold';
                icon = `<span class="w-4 h-4 rounded-full bg-rose-600 text-white flex items-center justify-center text-[10px]">✕</span>`;
              } else {
                btnCls = 'bg-slate-50 opacity-60 border-slate-200 text-slate-500';
              }
            }

            return `
              <button onclick="selectC9U2QuizOption(${qIdx}, ${optIdx})" ${answered ? 'disabled' : ''} class="p-3 rounded-2xl border text-left transition flex items-center gap-2 ${btnCls} cursor-pointer">
                ${icon}
                <span>${opt}</span>
              </button>
            `;
        ...
        ${answered ? `
          <div class="p-3 rounded-2xl ${isCorrect ? 'bg-emerald-50/80 border border-emerald-200 text-emerald-900' : 'bg-rose-50/80 border border-rose-200 text-rose-900'} text-xs space-y-1">
            <div class="font-bold flex items-center gap-1.5">
              <span>${isCorrect ? '🎉 सही उत्तर!' : '⚠️ उत्तर मिलेन!'}</span>
            </div>
...
function selectC9U2QuizOption(qIdx, optIdx) {
  if (c9u2QuizAnswers[qIdx] !== undefined) return;
  c9u2QuizAnswers[qIdx] = optIdx;
  renderC9U2Quiz();
}

function updateC9U2QuizScore() {
  const badge = document.getElementById('c9u2-quiz-score-badge');
  if (!badge) return;

  let correctCount = 0;
  Object.keys(c9u2QuizAnswers).forEach(qIdx => {
    if (c9u2QuizAnswers[qIdx] === C9U2_QUIZ_DATA[qIdx].correct) {
      correctCount++;
    }
  });

  const nepaliDigits = ['०', '१', '२', '३', '४', '५', '६', '७', '८', '९', '१०'];
  badge.textContent = `${nepaliDigits[correctCount]} / १०`;
}

function resetC9U2Quiz() {
  c9u2QuizAnswers = {};
  renderC9U2Quiz();
}
...
function renderFarmStepperC9U3() {
  ...
  container.innerHTML = C9U3_FARM_STAGES_DATA.map((s, idx) => {
    ...
    return `<button onclick="setFarmStageC9U3(${s.step})" class="w-8 h-8 rounded-xl border text-xs transition flex items-center justify-center cursor-pointer ${btnCls}">${s.step}</button>`;
  }).join('');
}
...
function setFarmStageC9U3(stageNum) {
  ...
  if (badgeEl) badgeEl.textContent = `चरण ${data.step} / ६`;
